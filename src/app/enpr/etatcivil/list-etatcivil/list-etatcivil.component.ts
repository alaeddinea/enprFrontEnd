...
import { Etatcivil } from '../../domain/etatcivil';

@Component({
  selector: 'app-list-etatcivil',
  templateUrl: './list-etatcivil.component.html',
  styleUrls: ['./list-etatcivil.component.scss']
})
export class ListEtatcivilComponent implements OnInit {
  ...
  edit(entity: Etatcivil): void {
    window.localStorage.removeItem('editEtatcivilId');

    window.localStorage.setItem('editEtatcivilId', entity.id.toString());
    this.router.navigate(['enpr/edit-etatcivil']);
  }

  addConge(): void {
    this.router.navigate(['enpr/add-etatcivil']);
  }
}
